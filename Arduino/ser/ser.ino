...
int fifthSensor = 0;
int sixSensor = 0;
int digSensor = 0;// digital sensor
...
void loop() {
  // if we get a valid byte, read analoprocessg ins:
  if (Serial.available() > 0) {
    ...
    fourthSensor = analogRead(A3)/4;

    fifthSensor = analogRead(A4);
    sixSensor = analogRead(A5);
    // read  switch, map it to 0 or 255L
//     digitalWrite(HIGH)
   //digSensor = map(digitalRead(2), 0, 1, 0, 255);
    // send sensor values:
